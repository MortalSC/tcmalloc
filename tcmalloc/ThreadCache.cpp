...
void ThreadCache::Deallocate(void* free_ptr, size_t size) {
	...
	// 内存块数量控制！过多的内存需归还到下一层
	// 策略：当链表的总长度， 党羽 一次批量申请的内存个数后就启动向下归还
	if (_freeListSet[index].GetSize() >= _freeListSet[index].MaxSize() ) {
		ListTooLong(_freeListSet[index], size);
	}
}
...
void* ThreadCache::FetchFromContralCache(size_t index, size_t size) {
	// 慢开始策略：第一次多分配（少量）
	size_t batchSize = min(_freeListSet[index].MaxSize(), SizeClass::NumMoveSize(size));
	...
	if (actualSize == 1) {
		...
		_freeListSet[index].PushRange(GetNextObj(start), end, actualSize-1);
		//return start;
	}
